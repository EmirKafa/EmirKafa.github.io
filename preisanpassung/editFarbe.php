...
function editFarbe() {
    ...
    if($stmt->rowCount() > 0) {
        while($res = $stmt->fetch()) {
            echo('<tr>
                    <td>'.$res[1].'</td>
                    <td>'.$res[2].'</td>
                    <td><a href="editFarbe.php?id='.$res[0].'">Bearbeiten</a></td>
                        </tr>
                        ');
        }
    }
 ...
 function openPopup($id) {
    global $db;
    $stmt = $db->prepare("SELECT * FROM farben WHERE id = :id");
 ...
 function saveEntry($service, $price) {
    global $db;
    $stmt = $db->prepare("UPDATE farben SET service = :service, price = :price WHERE id = :id");
    $stmt->bindParam("service", $service);
    $stmt->bindParam("price", $price);
    $stmt->bindParam("id", $_GET["id"]);
    $stmt->execute();
    header("Location: editFarbe.php");
 }
